...
这篇文章的主线不是“哪种 PEFT 方法参数最少”，而是：PEFT 方法本质上是在选择训练图和部署形态。

LoRA、Prefix-Tuning、P-Tuning、QLoRA 的差异不只在可训练参数量，而在它们如何改变显存、上下文窗口、推理路径、adapter 管理、合并策略和回滚方式。省参数只是入口，部署边界才是决策依据。
